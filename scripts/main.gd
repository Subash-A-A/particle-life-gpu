...
func _generate_particles():
	for i in range(NUM_PARTICLES):
		particles_pos.append(Vector2(randf() * get_viewport_rect().size.x, randf() * get_viewport_rect().size.y))
		particles_vel.append(Vector2.ZERO)
		particles_color.append(randi_range(0, NUM_COLORS - 1))
...
func _setup_compute_shader():
	...
	bindings = [particles_pos_uniform, particles_vel_uniform, particles_color_uniform, params_uniform, interaction_forces_uniform, interaction_distances_uniform, particles_data_buffer_uniform]
...
func _generate_float_buffer(data) -> RID:
	var data_buffer_bytes : PackedByteArray = PackedFloat32Array(data).to_byte_array()
